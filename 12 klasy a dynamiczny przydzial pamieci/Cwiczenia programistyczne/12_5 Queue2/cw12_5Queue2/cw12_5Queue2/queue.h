...
public:
	Queue(int qs = Q_SIZE);									// kolejka z limitem
	~Queue();
	bool isempty() const;
	bool isfull() const;
	int quecount() const;
...
// Kolejka nie dluzsza niz minuta
// znajdz wartosc srednia liczbe klientow na godizn, w ktorej sredni czas oczekiwania w kolejce wynisie minute po probach co najmniej 100 godzinnych